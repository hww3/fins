...
string simple_macro_field_describe(Fins.Template.TemplateData data, mapping|void arguments)
{
	...
	string ed = request->controller->make_value_describer(field, 
														 (item?item[field]:UNDEFINED), item);
...
string simple_macro_field_editor(Fins.Template.TemplateData data, mapping|void arguments)
{
	...
	string ed = request->controller->make_value_editor(field,	
                       orig[field] || (item?item[field]:UNDEFINED), item);	
